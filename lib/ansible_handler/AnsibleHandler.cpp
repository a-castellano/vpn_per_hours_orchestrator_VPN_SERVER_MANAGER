...
//contructor
AnsibleHandler::AnsibleHandler( boost::shared_ptr<std::string> token, DatabaseHandler *db, const std::string &serverIP )
:token(token), db(db), serveriIP(serverIP)
{
  time_t now;
  tm *ltm;
  ...
  // setting filename
  memoryLock.getLock();
  this->filename = std::string("serverName_") + std::to_string(1900 + ltm->tm_year) + std::string("-");
  ...
  memoryLock.getLock();
  this->filename = this->filename + std::to_string(ltm->tm_mon) + std::string("-");
  memoryLock.releaseLock();
  ...
  memoryLock.getLock();
  this->filename = this->filename + std::to_string(ltm->tm_mday) + std::string("_");
  memoryLock.releaseLock();
  ...
  memoryLock.getLock();
  this->filename = this->filename + std::to_string(ltm->tm_hour) + std::string(":");
  memoryLock.releaseLock();
  ...
  memoryLock.getLock();
  this->filename = this->filename + std::to_string(ltm->tm_min) + std::string(":");
  memoryLock.releaseLock();
...
bool AnsibleHandler::run()
{
  ...
  std::string chap_secrets_filename = std::string(CHAP_SECRETS_FILENAME) + this->filename;
  std::string inventory_filename = std::string(INVENTORIES_FILENAME) + this->filename;
...
memoryLock.releaseLock();
  ...
  for (unsigned int userID : VPNusers) {
    ...
    memoryLock.getLock();
    log = boost::make_shared<std::string>(user_id_string + std::to_string(userID));
    memoryLock.releaseLock();
